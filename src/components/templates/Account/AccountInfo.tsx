...
export const AccountInfo = () => {
    ...
    const setSubmit: SubmitHandler<AccountSchemaType> = (values) => {
        ...
        dispatch(updateNguoiDungThunk(UserUpdate)).unwrap().then(() => {
            dispatch(getUserByAccessTokenThunk()), toast.success('Cập nhật tài khoản thành công'), toast.error('Xin vui lòng chờ 10 giây để hệ thống cập nhật lại thông tin', { autoClose: 10000 }), toast.error('trong quá trình này quý khách không nên ấn "Cập nhật"', { autoClose: 10000 })
        })
            .catch(() => {
