  ...
  const setSubmit: SubmitHandler<PasswordSchemaType> = (values) => {
    ...
    if (matKhau === infoUser?.matKhau) {
      if (matKhauChanged1 === matKhauChanged2) {
        ...
        dispatch(updateNguoiDungThunk(infoUserDaDoiMatKhau)).unwrap().then(() => { dispatch(getUserByAccessTokenThunk()), toast.success('Đổi mật khấu thành công'), toast.error('Xin vui lòng chờ 10 giây để hệ thống cập nhật lại thông tin trong quá trình này quý khách không nên ấn "Cập nhật"', { autoClose: 10000 }) }).catch(() => { toast.error('Vui lòng F5 để load lại trang web') })
      }
    }
  }
  return (
